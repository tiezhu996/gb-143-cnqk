...
- 投诉处理、后台调整和排行榜
- 每人每个记录日期 8 小时有效工时上限：
  - 同一天补报多笔时，按当天入库顺序分配 8 小时额度，超出部分保留在原记录并标记为超额
  - 超额工时不计积分、不增加服务次数、不影响信用评价
  - 单条创建与批量导入均生效；爽约记录不占用 8 小时额度
  - 撤销记录时原记录保留（`status=revoked`），按当天顺序把后续记录的有效工时补回，并重算积分、等级、徽章、服务次数与信用分
  - 服务记录列表可查询有效工时 `valid_hours`、超额工时 `overtime_hours`、记录状态 `status/effective_status` 及当天重算结果
  - 历史数据在服务首次启动时自动补跑一次每日上限并对账积分（幂等）
